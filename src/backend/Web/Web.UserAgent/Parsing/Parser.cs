...
namespace Web.UserAgent.Parsing
{
    #region Imports
    ...
    public class ClientInfo : IUAParserOutput
    {
        ...
        {
            if (!String.IsNullOrWhiteSpace(UA.Family) && !String.IsNullOrWhiteSpace(OS.Family))
                return string.Format("{0} on {1}", UA.Family, OS.Family);
            if (!String.IsNullOrWhiteSpace(OS.Family))
                return OS.Family;
            if (!String.IsNullOrWhiteSpace(UA.Family))
                return UA.Family;
            return null;
        }
    }
    ...
    /// </summary>
    public sealed class Parser : IUserAgentParser
    {
        ...
        public static Parser GetDefault()
        {
            ...
            var resourceStream = assembly.GetManifestResourceStream("Web.UserAgent.Regexes.yaml");
